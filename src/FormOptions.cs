...
namespace gInk
{
	public partial class FormOptions : Form
	{
        ...
        private void FormOptions_Load(object sender, EventArgs e)
		{
            ...
            MobileInputUrlEd.Text = Root.MobileInput_Url;
            MobileInputPwdEd.Text = Root.MobileInput_Password;
            int mapIdx = MobileInputMapCb.Items.IndexOf(Root.MobileInput_Mapping);
            MobileInputMapCb.SelectedIndex = mapIdx >= 0 ? mapIdx : 0;
            MobileInputConnTypeCb.SelectedIndex = MobileInputConnTypeCb.Items.IndexOf(Root.MobileInput_ConnType);
            MobileInputAdbPathEd.Text = Root.MobileInput_AdbPath;
            MobileInputBleNameEd.Text = Root.MobileInput_BleServiceName;
...
//                for(int i=0;i<dlg.Images.Images.Count;i++)
        ...
        private void MobileInputUrlEd_Validating(object sender, CancelEventArgs e)
        {
            Root.MobileInput_Url = MobileInputUrlEd.Text;
            Root.MobileInput_Password = MobileInputPwdEd.Text;
            Root.RestartMobileServer();
        }

        private void MobileInputUrlEd_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == '\r')
            {
                this.SelectNextControl(this.ActiveControl, true, true, true, true);
        ...
        private void MobileInputMapCb_SelectedIndexChanged(object sender, EventArgs e)
        {
            string sel = MobileInputMapCb.SelectedItem?.ToString() ?? "FullScreen";
            Root.MobileInput_Mapping = sel;
        }

        private void MobileInputConnTypeCb_SelectedIndexChanged(object sender, EventArgs e)
        {
            string sel = MobileInputConnTypeCb.SelectedItem?.ToString() ?? "WiFi";
            Root.MobileInput_ConnType = sel;
            if (sel == "USB")
                Root.SwitchToUsbMode();
            else if (sel == "WiFi")
                Root.SwitchToWifiMode();
            else if (sel == "Bluetooth")
                Root.SwitchToBluetoothMode();
        }

        private void MobileInputAdbPathEd_Validating(object sender, CancelEventArgs e)
        {
            Root.MobileInput_AdbPath = MobileInputAdbPathEd.Text;
        }

        private void MobileInputAdbPathEd_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == '\r')
            {
                this.SelectNextControl(this.ActiveControl, true, true, true, true);
        ...
        private void MobileInputAdbReverseBtn_Click(object sender, EventArgs e)
        {
            int port = Root.RunAdbReverse();
            if (port > 0)
                MessageBox.Show($"ADB reverse successful! Phone can connect to ws://localhost:{port}/", "USB Connection", MessageBoxButtons.OK, MessageBoxIcon.Information);
            else
                MessageBox.Show("ADB reverse failed! Ensure phone is connected via USB with USB debugging enabled and ADB is installed.", "USB Connection Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }

        private void MobileInputBleNameEd_Validating(object sender, CancelEventArgs e)
        {
            Root.MobileInput_BleServiceName = MobileInputBleNameEd.Text;
            if (Root.MobileInput_ConnType == "Bluetooth")
                Root.RestartMobileServer();
        }

        private void MobileInputBleNameEd_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == '\r')
            {
                this.SelectNextControl(this.ActiveControl, true, true, true, true);
        ...
        private void MobileInputQrBtn_Click(object sender, EventArgs e)
        {
            try
            {
                string configStr = QRCodeHelper.GenerateConfigString(Root);
                using (Bitmap qrBmp = QRCodeHelper.Generate(configStr, 8))
                {
                    FormQrDisplay qrForm = new FormQrDisplay(qrBmp, configStr);
                    qrForm.ShowDialog();
                }
            }
            catch (Exception ex)
