...
namespace gInk
{
    ...
    public class Root
    {
        ...
        public double MobileInput_CustomX = 0;
        public double MobileInput_CustomY = 0;
        public double MobileInput_CustomWidth = 1;
        public double MobileInput_CustomHeight = 1;
        public WebSocketServer MobileWSServer;
        public BluetoothLEServer MobileBLEServer;
        public string MobileInput_BleServiceName = "ppInk";
        ...
        public Root()
		{
            ...
            */
            ...
            if (MobileInput_Url != "")
            {
                ...
                {
                    Console.WriteLine("Mobile input server init error: " + ex);
                }
            }

            // Mobile input BLE server
            if (MobileInput_ConnType == "Bluetooth")
            {
                try
                {
                    MobileBLEServer = new BluetoothLEServer(this);
                    if (!MobileBLEServer.Start(MobileInput_BleServiceName))
                        Console.WriteLine("Mobile input BLE server failed to start");
                }
                catch (Exception ex)
                {
                    Console.WriteLine("Mobile input BLE server init error: " + ex);
                }
            }
        ...
        public void RestartMobileServer()
        {
            ...
            if (MobileInput_Url != "" && (MobileInput_ConnType == "WiFi" || MobileInput_ConnType == "USB"))
            {
                ...
                {
                    Console.WriteLine("Mobile input server restart error: " + ex);
                }
            }
            else
            {
            ...
            if (MobileInput_ConnType == "Bluetooth")
            {
                ...
                {
                    Console.WriteLine("Mobile input BLE server restart error: " + ex);
                }
            }
        }

        /// <summary>
        /// Runs 'adb reverse tcp:PORT tcp:PORT' so the phone can reach
        /// the local WebSocket server over USB. Returns the parsed port or -1 on error.
        /// </summary>
        public int RunAdbReverse()
        {
            int port = -1;
            try
            {
                var uri = new Uri(MobileInput_Url);
                port = uri.Port;
            }
            catch { }
            ...
            if (port <= 0)
            {
                // Try parsing port from raw string
                var parts = MobileInput_Url.Split(':');
                if (parts.Length >= 2 && int.TryParse(parts[parts.Length - 1].TrimEnd('/'), out int p))
                    port = p;
            }
            ...
            try
            {
                ...
                using (Process proc = Process.Start(psi))
                {
                    ...
                    Console.WriteLine($"ADB reverse output: {output}{err}");
                    return proc.ExitCode == 0 ? port : -1;
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine("ADB reverse failed: " + ex.Message);
                return -1;
            }
        }

        /// <summary>
        /// When switching to USB mode, auto-configure the URL to localhost:<port>
        /// </summary>
        public void SwitchToUsbMode()
        {
            MobileInput_ConnType = "USB";
            // Extract port from current URL and set to localhost
            int port = -1;
            try
            {
                var uri = new Uri(MobileInput_Url);
            ...
            if (port > 0)
            {
                MobileInput_Url = $"http://localhost:{port}/";
                RestartMobileServer();
            }
        }

        /// <summary>
        /// When switching to WiFi mode, auto-configure the URL to the local IP
        /// </summary>
        public void SwitchToWifiMode()
        {
            MobileInput_ConnType = "WiFi";
            // Extract port and use local IP
            int port = -1;
            try
            {
                var uri = new Uri(MobileInput_Url);
            ...
            if (port > 0)
            {
                ...
                if (!string.IsNullOrEmpty(localIP))
                    MobileInput_Url = $"http://{localIP}:{port}/";
                else
                    MobileInput_Url = $"http://0.0.0.0:{port}/";
                RestartMobileServer();
            }
        }

        /// <summary>
        /// Get the first non-loopback local IPv4 address
        /// </summary>
        ...
        /// <summary>
        /// Switch to Bluetooth mode and restart BLE server
        /// </summary>
        public void SwitchToBluetoothMode()
        {
            MobileInput_ConnType = "Bluetooth";
            RestartMobileServer();
        }
				...
				)
				{
                    ...
                    switch (sName)
                    {
                        ...
                        case "RESTSERVER_URL":
                            APIRestUrl = sPara;
                            break;
                        case "MOBILE_INPUT_CONNTYPE":
                            MobileInput_ConnType = sPara;
                            break;
                        case "MOBILE_INPUT_URL":
                            MobileInput_Url = sPara;
                            break;
                        case "MOBILE_INPUT_ADBPATH":
                            MobileInput_AdbPath = sPara;
                            break;
                        case "MOBILE_INPUT_BLESERVICE":
                            MobileInput_BleServiceName = sPara;
                            break;
                        case "MOBILE_INPUT_PWD":
                            MobileInput_Password = sPara;
                            break;
                        case "MOBILE_INPUT_MAPPING":
                            MobileInput_Mapping = sPara;
                            break;
                        case "MOBILE_INPUT_CUSTOM":
                            string[] cm = sPara.Split(',');
                            if (cm.Length == 4)
                            {
                                double.TryParse(cm[0], out MobileInput_CustomX);
                                double.TryParse(cm[1], out MobileInput_CustomY);
                                double.TryParse(cm[2], out MobileInput_CustomWidth);
                                double.TryParse(cm[3], out MobileInput_CustomHeight);
                            }
                            break;
				...
				sPara = InkVisibleEnabled?"True":"False";
  				break;
                        ...
                        case "RESTSERVER_URL":
                            ...
                            break;
                        case "MOBILE_INPUT_CONNTYPE":
                            sPara = MobileInput_ConnType;
                            break;
                        case "MOBILE_INPUT_ADBPATH":
                            sPara = MobileInput_AdbPath;
                            break;
                        case "MOBILE_INPUT_BLESERVICE":
                            sPara = MobileInput_BleServiceName;
                            break;
                        case "MOBILE_INPUT_URL":
                            sPara = MobileInput_Url;
                            break;
                        case "MOBILE_INPUT_PWD":
                            sPara = MobileInput_Password;
                            break;
                        case "MOBILE_INPUT_MAPPING":
                            sPara = MobileInput_Mapping;
                            break;
                        case "MOBILE_INPUT_CUSTOM":
                            sPara = MobileInput_CustomX.ToString() + "," + MobileInput_CustomY.ToString() + "," + MobileInput_CustomWidth.ToString() + "," + MobileInput_CustomHeight.ToString();
                            break;
                        case "IMAGESTAMP_SIZE":
